...
let exclude = (await readFile(".gitignore", { encoding: "utf8" }))
  .split("\n")
  .filter((line) => !line.startsWith("#") && line != "");

await Promise.all(
  (await Array.fromAsync(glob(join(SRC_ROOT, "**"), { exclude }))).map(
    async (src) => {
      if (extname(src) == ".md") {
        let dest = join(
          DEST_ROOT,
          ...
          element(element) {
            ...
            element.onEndTag((endTag) => {
              if (alertType) {
                ...
                endTag.after(
                  /* HTML */ `<blockquote>${alertText}</blockquote>`,
                  {
                    html: true,
                  },
                );
        ...
        rewriter.on("h1 *, h2 *, h3 *, h4 *, h5 *, h6 *", {
          element(element) {
            ...
            headingContent += `<${element.tagName}${Array.from(
              element.attributes,
            )
              .map(([k, v]) => ` ${k}="${escape(v)}"`)
              .join("")}>`;
        ...
        rewriter.on("runnable-code code", {
          element(element) {
            runnableCodeFlag =
              element.getAttribute("class")?.match(/language-(.+)/)?.[1] ??
              null;
          },
        });
        ...
        if (!datePublished || !dateModified) {
          ...
          if (committerDates.length) {
            if (!datePublished)
              datePublished = new Date(committerDates.at(-1)!);
