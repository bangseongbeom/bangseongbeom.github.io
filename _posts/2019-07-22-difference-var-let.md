...
호이스팅이 존재하는 이유는 **함수 호이스팅** 때문입니다. 함수 호이스팅이 없다면, 다시 말해 `let`과 유사하게 함수가 코드 상에서 출현할 때 선언된다면, A 함수에서 B 함수를 호출할 때 B 함수가 코드 순서 상 반드시 먼저 나와야 합니다. 만약 함수 간에 서로가 서로를 호출하는 코드가 많을 경우 함수의 순서를 언제나 신경써야겠죠. 반면 호이스팅이 있을 경우 모든 함수 선언이 코드의 첫부분에 존재하는 것처럼 여겨지기 때문에 **코드 순서에 상관 없이 함수를 선언할 수 있게 됩니다[^avoid-painful-order].**
