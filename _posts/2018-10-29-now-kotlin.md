...
코틀린이 제공하는 가장 위대한 장점은, 자바에서 골칫거리였던 `NullPointerException`에 대한 안정성을 특유의 물음표(`?`) 문법으로 손쉽게 확보할 수 있다는 것입니다. 하지만 안드로이드 SDK는 자바로 짜여져 있어 모든 타입이 `null`을 가질 수 있습니다. 이러한 경우 논리 또는 문서 상으로는 `null`이 나올 수 없는데도 불구하고, 코틀린에서는 언제나 `null` 처리 코드를 추가해야 합니다.

물론 자바 메서드에 `@NotNull` 애너테이션을 붙여 `null`이 나오지 않는다는 것을 보장할 수도 있습니다. **여태까지의 안드로이드 SDK는 소수의 메서드에만 `@NotNull`이 적용된 상태라, 코틀린에서 불필요한 `null` 처리 코드를 작성하는 일이 많았죠.**

**다행스럽게도 안드로이드 파이(9.0)부터는 대부분의 SDK 메서드에 `@NotNull` 애너테이션을 붙이게 되었습니다[^android-pie-sdk-is-now-more-kotlin].** 심지어 코틀린만이 가진 고급 문법을 지원하기 위해 [KTX][ktx]라는 확장 기능도 만들었습니다. 2017년 코틀린을 안드로이드의 공식 언어로 채택했을 때에 비하면[^official-kotlin], 이제는 정말로 안드로이드가 코틀린을 지원한다고 볼 수 있게 되었습니다.
