...
코틀린이 그토록 빨리 인기를 누릴 수 있었던 이유는, 코틀린을 만든 주체가 안드로이드 스튜디오를 개발한 [제트브레인(JetBrains)][jetbrains]이기 때문입니다. 자사에서 개발한 언어이니 당연히 문법 강조나 자동 완성도 자바와 동일할 정도로 강력하게 지원하리라 생각할 수 있습니다.
...
코틀린의 장점 중 하나는 `NullPointerException`에 대한 안정성을 특유의 물음표(`?`) 문법으로 손쉽게 확보할 수 있다는 것입니다. 모든 타입이 `null`을 가질 수 있는 자바와는 달리, 코틀린은 타입 뒤에 물음표를 붙여야만 `null`을 가질 수 있게 했죠. 하지만 안드로이드 프레임워크는 자바로 짜여져 있습니다. 이러한 경우 논리 상으로는 `null`이 나올 수 없는데도 불구하고, 코틀린에서는 언제나 `null` 처리 코드를 추가해야 합니다.
