...
IP라는 프로토콜을 표현하기 위해 `AF_INET`과 `PF_INET`이라는 두 개의 방법이 존재한다는 것이 이상하게 느껴질 수 있습니다. 둘 중에 하나만 있어도 될텐데 말이죠.

이유는 이렇습니다. 아주 오래 전 소켓 프로그래밍을 설계할 당시에는, **하나의 주소 체계가 여러 프로토콜을 지원**할 것을 염두에 두고 만들었습니다[^bgnet-1]. 이를테면 IP 주소(192.168.0.1, 8.8.4.4같은 것들)가 IP 프로토콜뿐만 아니라 다른 프로토콜도 지원하는 식입니다.
