...
**물론, 드롭위자드가 스프링의 완벽한 대안이 될 수는 없습니다.** 자바라는 언어를 선택한다는 것은 대규모의 소프트웨어를 개발한다는 말과 같습니다. 그런 대규모 소프트웨어 개발에 필요한 요구 사항은 몹시 복잡합니다. 이러한 요구 사항을 효과적으로 수용하면서도 트렌드에 맞추어 빠르게 진화하는 프레임워크는 스프링이 유일합니다. 스프링 5.0에서 도입된 리액티브 프로그래밍[^reactive-support]과 코틀린[^kotlin-support] 지원이 이를 증명합니다.
